...
/**
 * Pre-defined character templates with images/icons.
 */
...
/**
 * CharacterStep - Step 2 of the wizard: Choose characters.
 * Users can select from templates or add custom character names.
 */
export default function CharacterStep({
  ...
  const toggleTemplate = (template) => {
    const exists = selectedCharacters.find((c) => c.id === template.id);
    if (exists) {
      onCharactersChange(selectedCharacters.filter((c) => c.id !== template.id));
    } else {
      onCharactersChange([
        ...selectedCharacters,
        {
    ...
    <div className="space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      <div className="text-center mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-lg">
          {isHebrew ? "בחר דמויות או צור משלך" : "Pick characters or create your own"}
        </p>
      </div>
      ...
      <div
        ...
        role="group"
        aria-label={isHebrew ? "בחר דמויות" : "Choose characters"}
      >
